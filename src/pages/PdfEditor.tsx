...
import { ArrowLeft, Download, Pencil, ExternalLink, FileText } from 'lucide-react';
...
const PdfEditor = () => {
  ...
  // This is the key change - we'll use Google's PDF viewer which works reliably
  ...
  useEffect(() => {
    ...
    // Create the Google Docs viewer URL if we have a PDF URL
    if (pdfUrl) {
      // Google Docs viewer URL
      setGoogleViewerUrl(`https://docs.google.com/viewer?url=${encodeURIComponent(pdfUrl)}&embedded=true`);
              ...
              {googleViewerUrl ? (
                <>
                  ...
                  <div className="p-2 bg-muted text-sm text-muted-foreground text-center">
                    PDF displayed using Google Docs viewer. For native viewing, use the <strong>Open in New Tab</strong> button.
                  </div>
                </>
              ) : (
